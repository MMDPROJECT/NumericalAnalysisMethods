...
def Rectangler_rule(f, a, b, n):
    del_x = (b - a) / n
    sum = 0
    for i in range(1, n):
        sum += del_x * f(a + i * del_x)
    integrated_value = sum + del_x  * f(a)
    return integrated_value

def Rectangler_rule_gui(f_expr, a, b, n):
    f = lambda x: eval_function(f_expr, x)
    return Rectangler_rule(f, a, b, n)
...
def calculate_integral():
    try:
        ...
        result = Rectangler_rule_gui(f_expr, a, b, n)
...
def plot_function(f_expr, a, b, n):
    try:
        ...
        ax.fill_between(x_trap, 0, y_trap, alpha=0.3, label="Rectangler Approximation")

        ax.set_xlabel("x")
        ax.set_ylabel("f(x)")
        ax.legend()
        ax.set_title("Function Plot and Rectangler Approximation")
...
root = tk.Tk()
root.title("Integration by Rectangler Rule")

...
def gaussian_quadratic_rule(f, a, b):
    result, error = quad(f, a, b)
    return result

def gaussian_quadratic_rule_gui(f_expr, a, b):
    f = lambda x: eval_function(f_expr, x)
    return gaussian_quadratic_rule(f, a, b)
...
def calculate_integral():
    try:
        ...
        result = gaussian_quadratic_rule_gui(f_expr, a, b)
...
def plot_function(f_expr, a, b, n):
    try:
        ...
        ax.fill_between(x_trap, 0, y_trap, alpha=0.3, label="Gaussian Quadratic Approximation")

        ax.set_xlabel("x")
        ax.set_ylabel("f(x)")
        ax.legend()
        ax.set_title("Function Plot and Gaussian Quadratic Approximation")
...
root = tk.Tk()
root.title("Integration by Gaussian Quadratic Rule")
